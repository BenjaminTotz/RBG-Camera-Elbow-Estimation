...
class TfPoseEstimator:
    ...
    row.write(0, "Date")
    row.write(1, "Time")
    row.write(2, "Shoulder X")              
    row.write(3, "Shoulder Y")
    row.write(4, "Elbow X")
    row.write(5, "Elbow Y")
    row.write(6, "Wrist X")
    row.write(7, "Wrist Y")
    ...
    def __init__(self, graph_path, target_size=(320, 240)):
        ...
        # load graph
        with tf.io.gfile.GFile(graph_path, 'rb') as f:  #changed to tf.io.gfile.GFile(graph_path, 'rb')
            graph_def = tf.compat.v1.GraphDef() #changed to tf.compat.v1.GraphDef()
            graph_def.ParseFromString(f.read()) #changed from graph_def.ParseFromString(f.read()) to tf.io.write_graph(graph_def, graph_dir, 'graph.pbtxt',as_text=False)
            #line 283 keeps throwing error: "Exception has occurred: TypeError; Expectedbinary or unidocde string, got None"
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                try:
                    ...
                    TfPoseEstimator.row.write(0, datetime.now().strftime('%Y-%m-%d'))
                    TfPoseEstimator.row.write(1, datetime.now().strftime('%H:%M:%S.%f')[:-2])
                ...
                try:
                    if (str(body_part.get_part_name()).strip()=="CocoPart.RShoulder"):
                           TfPoseEstimator.row.write(2, int(body_part.x * image_w + 0.5))
                           TfPoseEstimator.row.write(3, int(body_part.y * image_h + 0.5))
                        
                    if (str(body_part.get_part_name()).strip()=="CocoPart.RElbow"):
                            TfPoseEstimator.row.write(4, int(body_part.x * image_w + 0.5))
                            TfPoseEstimator.row.write(5, int(body_part.y * image_h + 0.5))
                            
                    if (str(body_part.get_part_name()).strip()=="CocoPart.RWrist"):
                            TfPoseEstimator.row.write(6, int(body_part.x * image_w + 0.5))
                            TfPoseEstimator.row.write(7, int(body_part.y * image_h + 0.5))
                except:
                    print("time not printed")   
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                try:
                    TfPoseEstimator.row.write(8, str(round(degree)))  
                except:
                    
                    #TfPoseEstimator.book.save("test.xls")
                    #attempting to save in a different location
                    TfPoseEstimator.book.save("C:\Code_Projects\Data\Round_2\Coronal_6.xls")     
                    #Error msg: Exception has occurred: PermissionError [Errno 13] Permission denied: 'test.xls'
                    #solution: the xls file cannot be saved to the active folder
